...
class Transactions:
    ...
    def add_ticker(self, tic_name, tic_object):
        Transactions.tickers_list[tic_name] = tic_object
...
def main():
    ...
    tickers_objects = {} # dictionary database where the key is a string and the value is of a class Ticker
    ...
    for k, v in list(tickers_data.items()):
        if v['Assigned'] == True:
            del tickers_data[k]
            # print(tickers_data.pop(k))
            # if tmp in tickers_data.keys():
            #     print('Still there')
